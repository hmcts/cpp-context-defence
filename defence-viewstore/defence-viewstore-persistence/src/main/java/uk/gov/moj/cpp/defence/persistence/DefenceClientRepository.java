...
public interface DefenceClientRepository extends EntityRepository<DefenceClient, UUID> {
    ...
    @Query(value = "select dc FROM DefenceClient dc INNER JOIN  DefenceCase c ON  c.id = dc.caseId WHERE upper(dc.firstName) = upper(:firstName) and TRIM(upper(dc.lastName)) = upper(:lastName) and "
